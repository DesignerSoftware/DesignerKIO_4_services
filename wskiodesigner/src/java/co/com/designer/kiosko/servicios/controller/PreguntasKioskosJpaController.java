 ...
 * @author Lenovo
 */
public class PreguntasKioskosJpaController implements Serializable {

    public PreguntasKioskosJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    private List<PreguntasKioskos> findPreguntasKioskosEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            Query q = em.createQuery("select object(o) from PreguntasKioskos as o");
            if (!all) {
    ...
    public int getPreguntasKioskosCount() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select count(o) from PreguntasKioskos as o");
